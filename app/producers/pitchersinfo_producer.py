...
def process_probable_pitchers_data(data):
    ...
    if 'probablePitchers' in data['gameData']:
        try:
            ...
            pitchers_info['game_date'] = game_date
            pitchers_info['game_time'] = game_time
            pitchers_info.columns = [f"probablePitchers_{col}" if col not in ['game_id', 'game_date', 'game_time'] else col for col in pitchers_info.columns]
            return pitchers_info
        except Exception as e:
            logger.error(f"Error processing probablePitchers: {e}")
            return None
    return None
