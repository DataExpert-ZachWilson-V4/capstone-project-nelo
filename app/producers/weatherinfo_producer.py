...
def process_weather_data(data):
    ...
    if 'weather' in data['gameData']:
        try:
            weather_info = pd.json_normalize(data['gameData']['weather'])
            weather_info['game_id'] = game_id
            weather_info['game_date'] = game_date
            weather_info['game_time'] = game_time
            weather_info.columns = [f"weather_{col}" if col not in ['game_id', 'game_date', 'game_time'] else col for col in weather_info.columns]
            return weather_info
        except Exception as e:
            logger.error(f"Error processing weather: {e}")
            return None
    return None

def stream_weather_data(start_game_number, end_game_number, topic):
    max_workers = 40  # Increased max_workers to 40
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_game = {executor.submit(fetch_and_process_game_data, game_number, topic): game_number for game_number in range(start_game_number, end_game_number + 1)}
...
def fetch_and_process_game_data(game_number, topic):
    producer = create_producer()
    data = fetch_game_data(game_number)
    if data:
        weather_data_frame = process_weather_data(data)
        if weather_data_frame is not None:
            records = weather_data_frame.to_dict(orient='records')
            for record in records:
                send_to_kafka(producer, topic, record, game_number)
            logger.info(f"Successfully sent weather data for game {game_number} to Kafka.")
        else:
            logger.info(f"No valid weather data for game {game_number}.")
    else:
...
def main():
    ...
    stream_weather_data(args.start_game, args.end_game, args.topic)
