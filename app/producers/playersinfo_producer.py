...
def process_players_data(data):
    ...
    if 'players' in data['gameData']:
        try:
            ...
            players_info['game_date'] = game_date
            players_info['game_time'] = game_time
            players_info.columns = [f"players_{col}" if col not in ['game_id', 'game_date', 'game_time'] else col for col in players_info.columns]
            return players_info
        except Exception as e:
